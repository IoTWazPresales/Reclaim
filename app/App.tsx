...
} from '@/lib/mindfulness/reactiveTriggersPreference';
...
function AppShell() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const enabled = await loadReactiveTriggersEnabled();
        if (!cancelled) {
          setReactiveTriggersEnabled(enabled);
        }
      } finally {
        if (!cancelled) {
          setReactiveTriggersReady(true);
        }
      }
    })();
    return () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeReactiveTriggersEnabled((enabled) => {
      setReactiveTriggersEnabled(enabled);
      setReactiveTriggersReady(true);
    });
    return unsubscribe;
