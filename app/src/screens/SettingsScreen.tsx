...
function getBuildProfileGuess(channel: string | null | undefined, isDev: boolean): 'development' | 'preview' | 'production' | 'unknown' {
...
export default function SettingsScreen() {
  ...
  // App update checking
  const { isUpdatePending, isChecking, checkForUpdates, applyUpdate, currentUpdateInfo } = useAppUpdates();
  const versionInfo = getAppVersionInfo();
  const buildProfileGuess = getBuildProfileGuess(versionInfo.channel, __DEV__);
  const shortUpdateId =
    typeof currentUpdateInfo?.updateId === 'string' && currentUpdateInfo.updateId.length > 8
      ? currentUpdateInfo.updateId.slice(0, 8)
      : currentUpdateInfo?.updateId ?? 'none';
  ...
  const saveMut = useMutation({
    ...
    onSuccess: async () => {
      ...
      await reconcileNotifications().catch((e) => { if (__DEV__) logger.debug('[SettingsScreen]', e); });
          ...
          {isDevOrPreview ? (
            <View style={{ marginTop: 12 }}>
              ...
              <Text variant="bodySmall" style={{ marginTop: 6, opacity: 0.8 }}>
                Profile: {buildProfileGuess} • Channel: {versionInfo.channel ?? 'unknown'} • Runtime: {versionInfo.runtimeVersion ?? 'unknown'}
              </Text>
              <Text variant="bodySmall" style={{ marginTop: 2, opacity: 0.8 }}>
                OTA enabled: {versionInfo.isUpdateEnabled ? 'yes' : 'no'} • Update ID: {shortUpdateId}
              </Text>
              <Text variant="bodySmall" style={{ marginTop: 2, opacity: 0.8 }}>
                Launch source: {currentUpdateInfo?.isEmbeddedLaunch ? 'embedded binary' : 'OTA update'} • Emergency launch:{' '}
                {currentUpdateInfo?.isEmergencyLaunch ? 'yes' : 'no'}
              </Text>
              {currentUpdateInfo?.createdAt ? (
                <Text variant="bodySmall" style={{ marginTop: 2, opacity: 0.8 }}>
                  Update created: {new Date(currentUpdateInfo.createdAt).toLocaleString()}
                </Text>
              ) : null}
            </View>
          ) : null}
