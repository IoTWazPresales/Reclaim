...
import {
  ...
  denormalizeEquipmentId,
  mapUiConstraintIdsToEngineInjuries,
} from '@/lib/training/setupMappings';
...
export default function TrainingSetupScreen({ onComplete }: TrainingSetupScreenProps) {
    ...
    if (profile.constraints) {
      ...
      if (profile.constraints.injuries) {
        profile.constraints.injuries.forEach((injury) => {
          ...
          if (injury.includes('shoulder') || injury.includes('rotator')) hydratedConstraints.push('shoulder_issues');
  ...
  const saveProfileMutation = useMutation({
    mutationFn: async () => {
      ...
      const engineInjuries = mapUiConstraintIdsToEngineInjuries(constraints);
    ...
    >
      ...
      >
        ...
        {step === 'schedule' && (
          <View>
            ...
            <View style={{ flexDirection: compactFooter ? 'column' : 'row', gap: 10 }}>
      ...
      <View
        style={{
          ...
          flexDirection: compactFooter ? 'column-reverse' : 'row',
      ...
      >
        <View
          style={{
            ...
            flexWrap: 'wrap',
            gap: appTheme.spacing.sm,
            flex: compactFooter ? 0 : 1,
            flexShrink: 1,
            minWidth: 0,
            alignItems: compactFooter ? 'stretch' : 'center',
          }}
        >
          <Button mode="outlined" onPress={prevStep} disabled={step === 'goals'} style={compactFooter ? undefined : { flexShrink: 0 }}>
            Back
          </Button>
          {profileQ.data && (
            <>
              <Button
                ...
                style={compactFooter ? undefined : { flexShrink: 1, minWidth: 0 }}
              >
                Exit
              </Button>
              {activeProgramQ.data && (
