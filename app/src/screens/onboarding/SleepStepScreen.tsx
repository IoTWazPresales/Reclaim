...
export default function SleepStepScreen() {
  ...
  const handleConnectIntegration = useCallback(
    ...
    [connectIntegration, refreshIntegrations, qc, visibleIntegrations, refreshInsights],
  );
    ...
    <View style={{ flex: 1, padding: 24, backgroundColor: theme.colors.background }}>
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View style={{ flex: 1, justifyContent: 'center' }}>
          ...
          <Text style={{ opacity: 0.8, marginBottom: 12, color: theme.colors.onSurfaceVariant }}>
            Connect your sleep source so your daily signal can include recovery, or continue and add sleep
            later in the app.
          </Text>
          <Text style={{ opacity: 0.75, marginBottom: 20, color: theme.colors.onSurfaceVariant, fontSize: 13 }}>
            {Platform.OS === 'android'
              ? 'Recommended on this device: Health Connect (Google).'
              : Platform.OS === 'ios'
                ? 'Recommended on this device: Apple Health.'
                : 'Connect a supported provider when available.'}
          </Text>
      ...
      <View style={{ paddingTop: 16 }}>
        <Button mode="outlined" onPress={goNext} style={{ marginBottom: 12 }} accessibilityLabel="Continue without connecting sleep">
