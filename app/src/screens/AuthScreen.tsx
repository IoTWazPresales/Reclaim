    ...
    try {
      ...
      if (url && redirectTo) {
        ...
        if (result.type === 'success' && result.url) {
          ...
          await setSessionFromDeepLink(result.url);
      ...
      const isVerifierError =
        messageLower.includes('verification expired') ||
        messageLower.includes('code verifier') ||
        messageLower.includes('verifier') ||
        messageLower.includes('non-empty') ||
        messageLower.includes('session expired');

      if (isVerifierError) {
        logger.warn('[AUTH] OAuth callback could not be completed, re-auth required');
      } else {
