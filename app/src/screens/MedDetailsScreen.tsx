...
export default function MedDetailsScreen() {
  ...
  // Adherence (30d) — log rows only (merged local + remote)
  ...
  const hasSchedule = !!(med.schedule?.times?.length && med.schedule?.days?.length);
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40, backgroundColor: theme.colors.background }}>
      ...
      <View style={{ marginBottom: 4 }}>
        ...
        >
          {curated ? 'Curated profile available' : 'General profile mode'}
        </Text>
      ...
      <SectionCard title="Recent doses" subtitle="Merged device + cloud history when signed in." theme={theme} appTheme={appTheme}>
        {logsQ.isLoading ? (
          ...
          <Text style={{ marginTop: 8, opacity: 0.75, color: theme.colors.onSurfaceVariant }}>No doses logged in the last 30 days.</Text>
        ) : (
          byDay.map(([day, rows]) => (
      ...
      {/* Logging summary */}
      <SectionCard title="30-day logging summary" subtitle="Taken vs logged rows in this window — not clinical validation." theme={theme} appTheme={appTheme}>
        <Text style={{ marginTop: 6, opacity: 0.85, color: theme.colors.onSurfaceVariant }}>
          Taken: {taken30}/{total30} ({pct30}%)
        </Text>
