...
export type RequestHealthSyncOptions = {
  ...
  /**
   * When false, skip React Query invalidation for sleep summary keys (caller may batch or suppress first-mount refetch).
   * Background/startup sync defaults to true so UI reflects acknowledged provider imports.
   */
  invalidateSummaryQueries?: boolean;
};
      ...
      if (!queuedConnectImportRun) {
        ...
        queuedConnectImportRun = (async () => {
          ...
          return requestHealthSync({
            ...
            force: true,
            minIntervalMs: 0,
            invalidateSummaryQueries: options.invalidateSummaryQueries,
          });
        })().finally(() => {
