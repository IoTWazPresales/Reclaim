...
describe('Session Runtime - Set Logging', () => {
  ...
  it('updateLoggedSetInRuntime patches completed set and allLoggedSets', () => {
    const plan = createTestPlan([{ exerciseId: 'bench_press' }]);
    let state = initializeRuntime('session_123', plan);
    state = logSet(state, 'bench_press', { setIndex: 1, weight: 60, reps: 8, rpe: 7 }).state;

    const patched = updateLoggedSetInRuntime(state, 'bench_press', 1, { weight: 62.5, reps: 8, rpe: 8 });

    const ex = patched.exerciseStates['bench_press'];
    expect(ex.completedSets).toHaveLength(1);
    expect(ex.completedSets[0].weight).toBe(62.5);
    expect(ex.completedSets[0].reps).toBe(8);
    expect(ex.completedSets[0].rpe).toBe(8);
    expect(patched.allLoggedSets).toHaveLength(1);
    expect(patched.allLoggedSets[0].weight).toBe(62.5);
  });
});
