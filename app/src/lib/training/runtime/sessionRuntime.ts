...
/**
 * Patch an already-logged set (user edited weight/reps/RPE) without inserting a duplicate log entry.
 * Keeps runtime autoreg / next-set suggestions aligned with displayed history.
 */
export function updateLoggedSetInRuntime(
  state: SessionRuntimeState,
  exerciseId: string,
  setIndex: number,
  ...
  const exerciseState = state.exerciseStates[exerciseId];
  if (!exerciseState) return state;

  const patchEntry = (s: SetLogEntry): SetLogEntry =>
    s.setIndex === setIndex
      ? {
          ...s,
          weight: patch.weight,
  ...
  const updatedExerciseState: ExerciseRuntimeState = {
    ...exerciseState,
    completedSets: exerciseState.completedSets.map(patchEntry),
  };

  const allLoggedSets = state.allLoggedSets.map((s) =>
    s.exerciseId === exerciseId && s.setIndex === setIndex ? patchEntry(s) : s,
  );
