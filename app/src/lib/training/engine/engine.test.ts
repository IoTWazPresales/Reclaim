...
import type { TrainingConstraints, UserState, GoalWeights, Exercise, MovementIntent, SessionState } from '../types';
...
describe('Prescription semantics guardrails (P0-4)', () => {
  it('applies 21s override reps/sets semantics', () => {
    const e = getExerciseById('21s');
    expect(e).not.toBeNull();
    const override = getExercisePrescriptionOverride(e!);
    expect(override.fixedTargetReps).toBe(21);
    expect(override.minSets).toBe(2);
  });

  it('prevents 1-set elbow-flexion base generation', () => {
    const session = buildSession({
      template: 'pull',
      goals: {
        build_muscle: 0,
      ...
      constraints: {
        availableEquipment: ['barbell', 'ez_bar', 'bench', 'rack', 'pull_up_bar'],
        injuries: [],
        forbiddenMovements: [],
        timeBudgetMinutes: 60,
      },
      userState: { experienceLevel: 'intermediate', estimated1RM: {} },
      intentOverrides: ['elbow_flexion'],
    });

    const elbowFlex = session.exercises.find((ex) => ex.intents.includes('elbow_flexion'));
    expect(elbowFlex).toBeDefined();
    expect(elbowFlex!.plannedSets.length).toBeGreaterThanOrEqual(2);
  });

  it('keeps 21s from collapsing to one set during fatigue adaptation', () => {
    const ex = getExerciseById('21s');
    ...
    const sessionState: SessionState = {
      ...
      plan: {
        ...
        constraints: {
          availableEquipment: ['barbell', 'ez_bar'],
          injuries: [],
          forbiddenMovements: [],
          timeBudgetMinutes: 52,
        },
        userState: { experienceLevel: 'intermediate', estimated1RM: {} },
        ...
        exercises: [
          {
            ...
            plannedSets: [
              { setIndex: 1, targetReps: 21, suggestedWeight: 20, restSeconds: 45 },
              { setIndex: 2, targetReps: 21, suggestedWeight: 20, restSeconds: 45 },
              { setIndex: 3, targetReps: 21, suggestedWeight: 20, restSeconds: 45 },
            ],
            decisionTrace: {
              intent: ['elbow_flexion'],
              goalBias: { build_strength: 1 },
              constraintsApplied: [],
    ...
    const adapted = adaptSession({ sessionState, reason: 'fatigue' });
