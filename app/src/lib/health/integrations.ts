...
/** Apple HealthKit permission bundle (Android HC uses HEALTH_CONNECT_DEFAULT_METRICS). */
...
async function connectGarmin(): Promise<{ success: boolean; message?: string }> {
  const message =
    'Reclaim does not connect to Garmin in this release. On Android, use Health Connect; on iPhone, use Apple Health to sync sleep and activity.';
...
async function connectHuawei(): Promise<{ success: boolean; message?: string }> {
  const message =
    'Reclaim does not connect to Huawei Health in this release. On Android, use Health Connect where your data is available.';
