...
export async function backfillHealthConnectDailyHistoryToSupabase(days: number): Promise<{
  ...
  const canVitals = await healthConnectHasPermissions(['heart_rate']).catch(() => false);
...
export async function syncAll(): Promise<{ moodUpserted: number; meditationUpserted: number }> {
  ...
  return { moodUpserted: moodReplayed, meditationUpserted: medRows.length };
}
    ...
    try {
      if (hcConnected && hcAvailable) {
        const [hcHasActivity, hcHasVitals] = await Promise.all([
          ...
          healthConnectHasPermissions(['heart_rate']).catch(() => false),
        ]);
